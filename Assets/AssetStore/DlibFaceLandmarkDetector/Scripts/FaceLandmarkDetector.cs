...
#if !DLIB_DONT_USE_UNSAFE_CODE && UNITY_2018_2_OR_NEWER
...
namespace DlibFaceLandmarkDetector
{
    ...
    public class FaceLandmarkDetector : DisposableDlibObject
    {
        ...
        */
        ...
        public List<Rect> DetectClosest()
        {
            return  DetectClosestFace(0.0);
        }
        ...
        */
        ...
        public List<Rect> DetectClosestFace(double adjust_threshold)
        {
            ThrowIfDisposed();
            ...
            if (rects.Count == 0)
            {
                return new List<Rect>(); // No faces detected, return an empty list
            }

            // Assume screen size or image size is known, e.g., width and height
            float screenWidth = 1920; // Replace with actual screen width
            float screenHeight = 1080; // Replace with actual screen height
            float centerX = screenWidth / 2;
            float centerY = screenHeight / 2;

            Rect closestFace = rects[0];
            float minDistance = float.MaxValue;

            foreach (var rect in rects)
            {
                // Calculate the center of the rectangle
                float faceCenterX = rect.x + rect.width / 2;
                float faceCenterY = rect.y + rect.height / 2;

                // Calculate distance from the center of the screen
                float distance = (float)Math.Sqrt(Math.Pow(centerX - faceCenterX, 2) + Math.Pow(centerY - faceCenterY, 2));
            ...
            return new List<Rect> { closestFace };
        }
