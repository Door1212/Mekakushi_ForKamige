...
public class EnemyAI_patrol_Search : MonoBehaviour
{
    [SerializeField]
   ...
   EnemyAI_patrol enemyMove = default; // EnemyAI_moveスクリプトへの参照
    ...
    void Start()
    {
        enemyMove = transform.parent.GetComponent< EnemyAI_patrol>(); // 親オブジェクトにあるEnemyAI_moveスクリプトを取得
    }
            ...
            if (angle <= searchAngle) // プレイヤーが検索角度内にいる場合
            {
                ...
                if (!Physics.Linecast(transform.position + Vector3.up, target.transform.position + Vector3.up, obstacleLayer))
                {
                    ...
                    if (playerDistance <= searchArea.radius * catchDistanceMultiplier
                        ...
                        && enemyMove.state != EnemyAI_patrol.EnemyState.Catch)
                    {
                        enemyMove.SetState(EnemyAI_patrol.EnemyState.Catch); // キャッチ状態に変更
                    }
                    // プレイヤーが検索エリア内にいる場合
                    else if (playerDistance <= searchArea.radius
                              && playerDistance >= searchArea.radius * catchDistanceMultiplier
                            && enemyMove.state == EnemyAI_patrol.EnemyState.Idle)
                    {
